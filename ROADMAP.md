...
### 4. 分流配置重构 ✅ 已完成 2026-04-24
- 18 个内置规则分类 + 支持自定义，18 个内置出站组 + 支持自定义
- 结构化自定义规则（site_tags / ip_tags / domain_suffix / domain_keyword / ip_cidr 等字段）
- 3 个预设方案（minimal / balanced / comprehensive），可一键覆盖或通过 `?preset=` 即时覆盖
- rule-provider URL 前缀可覆写（Clash / Sing-box 各自独立）
- 支持 5 格式客户端（Clash / Sing-box / Surge / V2Ray / Shadowrocket）
- 首次升级自动导入老 `settings.custom_rules` 文本到结构化规则表
- 设计/计划：[specs/2026-04-24-routing-refactor-design.md](specs/2026-04-24-routing-refactor-design.md) · [plans/2026-04-24-routing-refactor.md](plans/2026-04-24-routing-refactor.md)
...
| 里程碑 | 包含 | 目标周期 |
|---|---|---|
| M1 真实流量 + 订阅安全 | #2 ~~#3~~ ~~#4~~ | 2-3 周 |
| M2 多节点集群 | #1 #8 #16 | 4-6 周 |
| M3 商业化闭环 | #5 #12 #6 | 4 周 |
| M4 架构升级 | #11 #7 #15 | 6 周 |
